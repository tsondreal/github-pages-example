export const GETTING_STARTED: string = 'GETTING_STARTED';
export const P2A: string = 'P2A';

export const SOFTWARE_TYPE_TOOL: string = 'SOFTWARE_TYPE_TOOL';
export const SOFTWARE_TAXONOMY: string = 'SOFTWARE_TAXONOMY';

export const SERVICE_GUIDE: string = 'SERVICE_GUIDE';
export const SERVICE_MANUAL: string = 'SERVICE_MANUAL';

export const EA_OFFICE: string = 'EA_OFFICE';

export const TABS_TEMPLATE: string = 'TABS_TEMPLATE';